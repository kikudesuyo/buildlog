...
func ListDeletedPosts(ctx context.Context) ([]entity.DBTablePost, error) {
	...
	err := db.WithContext(ctx).Unscoped().Where("deleted_at IS NOT NULL").Order("deleted_at DESC").Find(&posts).Error
	return posts, err
}
...
func RestorePost(ctx context.Context, id int64) error {
	db := library.GetDB(ctx)
	return db.WithContext(ctx).Unscoped().Model(&entity.DBTablePost{}).Where("id = ?", id).Update("deleted_at", nil).Error
}

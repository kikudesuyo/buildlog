<script lang="ts">
	...
	let isDirty = $derived(
		...
		JSON.stringify(tags) !== JSON.stringify(data.diary.tags || [])
	);
	...
	async function handleSave(statusVal: 'draft' | 'published') {
		...
		errorMessage = '';
		try {
			await updateDiary(data.diary.id, title, content, tags, statusVal);
			goto(resolve('/admin'));
		} catch {
	...
	function addTag() {
		const newTag = window.prompt('タグ名を入力してください：');
		if (newTag?.trim() && !tags.includes(newTag.trim())) tags = [...tags, newTag.trim()];
	}
			...
			<div class="flex flex-col gap-3">
				...
				<div class="flex flex-wrap items-center gap-2">
					{#each tags as tag (tag)}
						<span class="inline-flex items-center gap-1 rounded bg-surface-container-high px-3 py-1 text-body-sm text-on-surface-variant">
							{tag}
							<button type="button" onclick={() => removeTag(tag)} class="cursor-pointer text-[10px] font-bold transition-colors hover:text-error" aria-label={`${tag}を削除`}>×</button>
						</span>
					{/each}
					<button type="button" onclick={addTag} class="cursor-pointer rounded border border-dashed border-outline-variant/60 px-3 py-1 text-body-sm text-outline transition-all hover:border-primary hover:text-primary">+ タグを追加</button>
				</div>
			</div>
			<!-- 公開設定 -->
			<div class="flex flex-col gap-3">

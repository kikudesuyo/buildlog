<script lang="ts">
	...
	import { updateApp } from '$lib/api/client';
	...
	let isDirty = $derived(
		slug !== (data.appProject.slug || '') ||
		name !== data.appProject.name ||
		category !== data.appProject.category ||
		tagsText !== data.appProject.tags.join(', ') ||
		description !== data.appProject.description ||
		icon !== data.appProject.icon ||
		iconUrl !== (data.appProject.iconUrl || '') ||
		demoUrl !== (data.appProject.demoUrl || '') ||
		codeUrl !== (data.appProject.codeUrl || '')
	);
	...
	async function handleSave() {
		...
		try {
			await updateApp(Number(data.appProject.id), {
				slug: slug.trim(),
				name: name.trim(),
...
<svelte:head>
	<title>アプリ編集 — Buildlog Admin</title>
</svelte:head>

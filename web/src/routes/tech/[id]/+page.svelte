...
<div class="editorial-container mx-auto px-gutter pt-24 pb-20 max-w-[800px]">
	...
	<article class="flex flex-col gap-6">
		<header class="flex flex-col gap-4 border-b border-outline-variant/20 pb-8">
			...
			{#if data.tech.tags.length > 0}
				<div class="flex flex-wrap gap-1.5">
					{#each data.tech.tags as tag (tag)}
						<a
							href={resolve(('/tech?tag=' + encodeURIComponent(tag)) as `/tech?${string}`)}
							class="font-label-sm text-[11px] rounded-full bg-surface-container-high px-2 py-0.5 text-on-surface-variant transition-all hover:bg-primary hover:text-on-primary"
						>
							#{tag}
						</a>
					{/each}

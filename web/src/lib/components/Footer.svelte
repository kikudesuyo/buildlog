...
<footer class="bg-surface w-full border-t border-outline-variant/20 py-section-gap">
	...
	<button type="button" onclick={handleBuildlogTap} class="font-label-md text-label-md text-primary font-semibold">
		...
		<div class="flex flex-wrap items-center gap-2">
			...
			{#if xUrl}
				<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
				<a href={xUrl} target="_blank" rel="noopener noreferrer" class="font-label-sm text-label-sm flex min-h-11 items-center gap-2 rounded-lg px-3 text-on-surface-variant transition-colors hover:bg-surface-container-low hover:text-primary" title="X (Twitter)（外部サイト）" aria-label="X (Twitter)（外部サイト）">

...
const apiBaseUrl = (() => {
	const rawUrl = (env as { PUBLIC_API_BASE_URL?: string }).PUBLIC_API_BASE_URL || '';
...
function normalizeTags(tags: ApiPost['tags'] | undefined): string[] {
	return (tags || []).map((tag) => (typeof tag === 'string' ? tag : tag.name));
}

export async function fetchDiaryEntries(fetchFn: ApiFetch, filter: boolean | string = false): Promise<DiaryEntry[]> {
	const params = new URLSearchParams();
	if (filter === true) params.set('all', 'true');
	if (typeof filter === 'string' && filter) params.set('tag', filter);
...
export async function fetchTechFeed(fetchFn: ApiFetch, filter: boolean | string = false): Promise<{
...
export async function createDiary(title: string, content: string, tags: string[] = [], status: 'draft' | 'published' = 'draft'): Promise<DiaryEntry> {
	const response = await sendRequest<ApiObjectResponse<ApiPost>>('POST', '/diaries', { title, content, tags, status });
	return {
		id: response.data.id,
...
export async function updateDiary(id: number, title: string, content: string, tags: string[] = [], status: 'draft' | 'published' = 'draft'): Promise<DiaryEntry> {
	const response = await sendRequest<ApiObjectResponse<ApiPost>>('PUT', `/diaries/${id}`, { title, content, tags, status });
	return {
		id: response.data.id,
		title: response.data.title,
		content: response.data.content,
